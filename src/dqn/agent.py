...
class DQN:
    ...
    def Train_Qtable(self, iter_num=1000, target_update=10):
        ...
        self.recorder.info('dqn 训练中出现的最小时延：{},出现在第 {} 次训练中'.format(self.good['total_latency'], self.good['episode']))
        self.recorder.info(f"dqn 最短路线:{self.good['mstream_order']}")
        self.recorder.info('dqn 训练中出现的最大时延：{},出现在第 {} 次训练中'.format(self.bad['total_latency'], self.bad['episode']))
        self.recorder.info(f"dqn 最短路线:{self.bad['mstream_order']}")
